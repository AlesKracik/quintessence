# Spec Readback: auth-ui — v0.1.0

> Auto-generated by `tools/spec-readback.py` from `specs/auth-ui.json`. Do not edit; regenerate after spec changes.

**Status:** formalized  |  **Requirements:** 0/3 verified, 0/3 witnessed  |  **Invariants:** 0/1 verified  |  **Coverage:** matrix not run  |  **Open questions:** 0  |  **Last verified:** never

*Legend: ✓ verified — witness trace replayed green against real code · ◐ witnessed — proven possible in the model, not yet demonstrated in code · ✗ no witness — claimed behavior is UNREACHABLE in the model · ⏳ not checked yet · ⊘ skipped with justification (rejection-style requirement; an invariant carries the proof)*

## Purpose

Login flow UI: Home screen, Login form, Dashboard (auth-required), LockedNotice. The navigation graph is model-checked — Dashboard is provably unreachable without an Active session.

## ⚠ Needs Your Attention

- **Unchecked requirement** — UI-001
- **Unchecked requirement** — UI-002
- **Unchecked requirement** — UI-003

## Navigation

```mermaid
graph TB
  Home --> |click 'Sign in'| Login
  Login --> |submit valid credentials| Dashboard
  Login --> |submit when account locked| LockedNotice
  Login --> |submit invalid credentials (stays, shows error)| Login
  Dashboard --> |click logout| Home
  LockedNotice --> |after lockout expires (page refresh)| Home
  classDef auth_required fill:#fff5b1
  class Dashboard auth_required
```

## Screens

| Screen | Auth required | Purpose | Components |
|---|---|---|---|
| Home | no | Public landing; shows 'Sign in' for unauthenticated users. | Header |
| Login | no | Email/password form. | Header, LoginForm |
| Dashboard | yes | Logged-in landing. | Header |
| LockedNotice | no | Shown when login fails because the account is locked. | Header |

## UI Components

- **LoginForm** — fields: email, password; states: idle, submitting, error
- **Header** — visible when: always

## What the System Does

### Sign in — *User — User opens the login screen, signs in, and lands on the Dashboard — including the lockout failure branch.*

#### UI-001

⏳ While the user is on Home, when the user clicks 'Sign in', the system shall navigate to Login.

<details><summary>Quint action `click_sign_in`, witness predicate + trace</summary>

`specs/auth-ui.qnt:L42-L47` · model `3b187046b51a`

```quint
  action click_sign_in: bool = all {
    current == Home,
    current'       = Login,
    form'          = Idle,
    authenticated' = authenticated,
  }
```

**Witness predicate:** `current == Login` — true exactly when the behavior has happened.

</details>

*(step 2: [auth.REQ-001](auth.readback.md#req-001) — see [auth](auth.readback.md))*

#### UI-002

⏳ While the user is on Login, when the user submits valid credentials, the system shall navigate to Dashboard.

<details><summary>Quint action `submit_success`, witness predicate + trace</summary>

`specs/auth-ui.qnt:L51-L58` · model `3b187046b51a`

```quint
  action submit_success: bool = all {
    current == Login,
    not(authenticated),                       // not already logged in
    // Assumption: credentials would be checked against auth's User store.
    current'       = Dashboard,
    form'          = Idle,
    authenticated' = true,
  }
```

**Witness predicate:** `current == Dashboard` — true exactly when the behavior has happened.

</details>

*(step 4: [auth.REQ-003](auth.readback.md#req-003) — failure branch: repeated bad credentials lock the account — see [auth](auth.readback.md))*

*step 5 — locked-account counterpart of the submit step:*

#### UI-003

⏳  *(failure path)* While the user is on Login, if the user submits credentials for a Locked account, then the system shall navigate to LockedNotice.

<details><summary>Quint action `submit_locked`, witness predicate + trace</summary>

`specs/auth-ui.qnt:L61-L67` · model `3b187046b51a`

```quint
  action submit_locked: bool = all {
    current == Login,
    not(authenticated),
    current'       = LockedNotice,
    form'          = ErrorLocked,
    authenticated' = authenticated,
  }
```

**Witness predicate:** `current == LockedNotice` — true exactly when the behavior has happened.

</details>

## What Must Always Be True

- **INV-001** (`guardedDashboard`) — Dashboard is reachable only when authenticated == true. ('Shall always' statements are invariants, not requirements — Apalache proves them across all reachable states.) Criticality: critical. ⏳

## Reference

<details><summary>Concepts, architecture, decisions, resolved questions, traceability, verification history</summary>

**Actors:** User

**Architecture (resolved project ⊕ area):** TypeScript 5.4 Node.js 20 React Vitest pnpm · persistence: sql postgres Prisma

**Decisions:**

- **DEC-001** (accepted, 2026-05-15) — Model the navigation as a Quint state machine. Encode screens as a variant type, navigation as Quint actions, auth gating as invariants checked by Apalache. Rationale: Lets us prove 'Dashboard unreachable without auth' rather than relying on convention. Visual design stays in the design tooling — not formalized.

_No code generated yet. Run /spec-apply._

_Approval lives in PR history — audit trail: `git log --follow specs/auth-ui.json`_

</details>
