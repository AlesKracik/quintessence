...
The workflow runs on **five Claude Code commands**:
...
| `/spec-readback [area]` | Renders the human review document — generated by a tool, byte-identical for identical input, so its git diff *is* the review. |

Everything else — EARS structure, witness soundness, the completeness axes, modality, brownfield extraction, the tiering that lets the precision core run on Python alone — is described in the methodology.
