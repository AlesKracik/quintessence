...
- **One JSON file per area** at `specs/<name>.json`, with a Quint sidecar `specs/<name>.qnt`. Three kinds: `area` (functional), `contract` (cross-area invariants), `ui` (interactive surface — navigation modeled formally).
