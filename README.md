...
- **Semantic diff**: `tools/spec-diff.py <ref>` reports what *changed in what the system promises* — domains widened, witnesses lost, obligations created, and the blast radius of any decision whose ground moved. `--since` embeds it in the change readback.
...
- **Brownfield is the strong case**: an existing implementation is an oracle. Declare a `boundary` (what a replacement must preserve — and what it may do differently), account for every decision site in the code with `spec-extract-audit` (the one check that runs *code → spec*, because the branch nobody wrote down is exactly what regeneration gets wrong), then `/spec-apply --parallel` + `spec-record equiv` to drive the original and the regenerated implementation through the same sequences and diff them. Fidelity becomes a measurement instead of a judgement.
