...
Subcommands:
  ...
  change [slug]        → .spec/changes/<slug>.readback.md + refreshed
...
def landed_changes_for_id(root, area_name, item_id):
    ...
    out = []
    changes_dir = Path(root) / ".spec" / "changes"
    ...
    for p in sorted(changes_dir.glob("*.json")):
        ...
        for t in d.get("targets", []) or []:
            if t.get("name") == area_name and item_id in (t.get("ids") or []):
                out.append(d.get("change") or p.stem)
    return out
...
def reference_section(root, area, project):
    ...
    lines.append("_Approval lives in PR history — audit trail: "
                 "`git log --follow specs/" + f"{area.get('area')}.json`_")
    lines.append("")
    lines.append("</details>")
    return lines
...
def emit_area(root, area_name):
    area = load_json(Path(root) / "specs" / f"{area_name}.json")
    if area is None:
        sys.exit(f"ERROR: specs/{area_name}.json not found")
    ...
    lines = [f"# {title}: {area_name} — v{area.get('version', '?')}", "",
             f"> Auto-generated by `tools/spec-readback.py` from `specs/{area_name}.json`. "
...
def load_change(root, slug):
    ...
    manifest = load_json(Path(root) / ".spec" / "changes" / f"{slug}.json")
    if manifest is None:
        sys.exit(f"ERROR: .spec/changes/{slug}.json not found")
...
def change_grid(root, manifest):
    ...
    for t in manifest.get("targets", []) or []:
        area = load_json(Path(root) / "specs" / f"{t.get('name')}.json")
        if area is None:
...
def emit_change(root, slug):
    ...
    for t, area, _ph in grid:
        if area is None:
            attn.append(f"**{t.get('name')}** — specs/{t.get('name')}.json MISSING")
            continue
    ...
    for t, area, _ph in grid:
        ...
        for iid in ids:
            ...
            if kind_item is None:
                lines.append(f"- {iid} — _not found in specs/{name}.json_")
                continue
            key, item = kind_item
            if key == "req":
    ...
    write_doc(Path(root) / ".spec" / "changes" / f"{slug}.readback.md", lines)
...
def emit_project(root):
    ...
    loaded = {a: load_json(Path(root) / "specs" / f"{a}.json") for a in areas}
    journeys = load_journeys(root)
    ...
    for a, data in loaded.items():
        if data is None:
            attn.append(f"**{a}** — specs/{a}.json missing")
            continue
