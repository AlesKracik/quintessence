...
/spec-check [target] --steps 20                # override max_steps (the deep pass's ceiling)
/spec-check [target] --shallow-steps 5         # override shallow_steps (the first pass's bound)
/spec-check [target] --budget 1800             # override budget_seconds (cap on the WHOLE run)
...
- `.spec/project.json` — for Apalache settings (`timeout_seconds`, `max_steps`, `shallow_steps`, `budget_seconds`). `timeout_seconds` caps **one** check; `budget_seconds` (default 900) caps **the whole run**, so a 20-check area declares its cost before it starts instead of discovering it by waiting. `shallow_steps` (default 3) is the first pass's bound, `max_steps` the ceiling the second aims at. All four have working defaults — a project with no `apalache` block needs none of them.
...
2. **Generate/refresh the probe module** at `specs/<target>.probes.qnt` (see `templates/probes.qnt.template`). It imports the area module, instruments steps with ghost vars, and declares one negated probe per requirement, named `witness_<REQ_ID with - → _>`:
   ...
   - **Transition invariants** — any invariant declaring `over: "probes"` is emitted here too, as `val <quint_name>: bool = <invariants[].predicate>`, **positively**: a probe is negated so its counterexample is the witness, while a counterexample to one of these is a violation. The `_prev*` ghosts such a predicate reads are declared and snapshotted for it exactly as for a witness delta. It is authored in the area JSON because this module is generated and the ghosts are not in scope in the sidecar — see "Transition properties" below.
...
The runner does, deterministically: an advisory `quint run` pre-gate (seconds, writes `check_results.simulation` and never a verdict — `--only-simulate` for the fast loop while drafting, `--no-simulate` to skip), every invariant check (batched into one `quint verify` first, per-id with counterexample traces to `specs/<target>/traces/<ID>.cex.itf.json` when the batch isn't clean), every property as a temporal formula (`--temporal`, on TLC — no step bound, no ITF trace), every witness probe (`--init=initP --step=stepP`, violation trace = the witness), skip-if-fresh (a REQ already `witnessed` against the current `model_sha` is not re-proven), and all JSON write-backs: `check_results`, `formal_status`, `witness.{status,trace,checked_at,model_sha}` — written **as each check completes**, so a run stopped by Ctrl-C or by its budget keeps what it proved (`check_results.ran_at`, the field that means "this area was checked", is the one thing stamped only at the end). If it reports `quint` missing, run `tools/check-tooling.sh` for install hints — which also reports which of the optional flags (`run --backend`, `run --witnesses`, `verify --invariants`, `verify --temporal`) your quint has.

**Bounded checks run in two passes, not one.** A counterexample is shallow — the pre-gate falsifies invariants in milliseconds — while depth is only needed to *fail* to find one. So:

- **Pass 1 (shallow, `shallow_steps`)** runs every bounded invariant and every witness probe at the shallow bound. A counterexample here is **final**: its trace is recorded and it is never re-run, because depth cannot unfind it. A timeout or error is final for the same reason — running the same check deeper will not make it answer.
- **Pass 2 (deep, `max_steps`)** re-runs only what came back clean, for as long as `budget_seconds` lasts. Pass 1 itself is never budget-gated: it is the cheap half *and* the half that carries the findings, so every selected check always gets at least a shallow verdict.
- **Ordering:** pass 1 runs the invariants the pre-gate already falsified first (`check_results.simulation.invariants.falsified`) — guaranteed counterexamples, cheaply. Witness probes are ordered by the same signal inverted: a probe the simulator *hit* is about to be witnessed in milliseconds, a probe it never reached is the expensive tail. A run with `--no-simulate` has no such signal and keeps declaration order.
- **The ladder does not apply** to `proof: "inductive"` (unbounded), `proof: "structural"` (Alloy, finite scope), `proof: "smt"` (z3), or temporal properties (TLC, no step bound). A step ladder is meaningless for all four, and each keeps its existing single-pass path.
- **Server hygiene:** once any bounded check times out, the remaining ones add `quint verify --server=false`. Killing a timed-out check leaves its query inside the shared Apalache server, and without this the deep pass would inherit that congestion. Probe-gated — a quint without the flag behaves as before.

`--only` narrows the set of checks but still runs each survivor through the ladder. It is a tool for extracting one trace, not a substitute for the two passes — don't reach for a hand-written `--only` list to make a run cheaper.

Interpret its output:

- **`NO-WITNESS`** → report loudly: the behavior is unreachable — impossible guard, missing action, or bound too small. The line says which depth it was looked for at (`witness.steps`): a probe whose deep pass the budget stopped is unreachable *within the shallow bound only*, which is a far weaker statement than unreachable within the ceiling — never report the second when the record says the first. Offer: (a) inspect the guard, (b) raise `--steps` (or `--budget`, when the deep pass never ran), (c) for a rejection requirement, set `modality: "forbidden"` and `witness.enforced_by` to the enforcing invariant — the proof is an invariant, not a trace, and the recorder writes `status: "skipped"` itself (see METHODOLOGY → "Rejection requirements"); a prose `justification` also discharges but is not checkable. (a)/(b) are yours to act on; (c) is the one witness field you set by hand, and only with the user's confirmation.
- **`no-probe` / `no-probes-file`** → regenerate the probes module: `tools/spec-probes.py <target>`. Do NOT hand-roll it. The generator reads the area JSON and the sidecar's IR and fails rather than emitting a module that omits an action — the failure mode of writing it by hand is that a miscounted branch leaves actions out of `stepP`, and every probe pointing at them then silently cannot fire. It needs `formal_model.probe_domains` (one Quint set per parameter TYPE); if that is missing it prints the exact JSON to add.
- **`HOLLOW`** → the probe found a counterexample of ONE state: the predicate was already true before anything ran, so nothing was demonstrated. Report it as loudly as a NO-WITNESS — it is a false proof, not a weak one, and it comes back in seconds so it reads as healthy. Fix by adding `witness.delta.pre` (a pre-state over the `_prev*` ghosts that the step had to start from) and, usually, by sharpening a predicate that could not tell before from after. Then regenerate the probes and re-run.
- **`simulate` lines** → these come from the advisory `quint run` pre-gate, and they are **never** verdicts. `no violation in explored runs` is not `verified`; only the `quint verify` lines below it decide anything. Two are worth acting on: a `VIOLATION FOUND` tells you the bug is shallow enough that a random walk hit it (the Apalache line right below will carry the trace), and `0 traces` on a witness says no random execution ever reached that behavior — treat it as an early `NO-WITNESS` warning and start looking at the guard before the model checker confirms it.
- **Properties (PROP-NNN) honesty rule:** a PROP is a **temporal** formula, so it runs as `quint verify --temporal=<quint_name> --backend=tlc`, not as an invariant — Apalache's temporal support is partial, and the checker for this path is TLC (override in `.spec/project.json` → `quint.temporal_backend`). Report a TLC `verified` as *exhaustive over this model's finite state space*, and say the second half out loud: no step bound, but no instance larger than the model declares either, so widening the model's own domains is what strengthens it. TLC writes **no ITF trace**, so a PROP counterexample arrives with nothing to diagram — say that rather than leaving the user hunting for a trace file. On `timeout` (state explosion is TLC's failure mode), suggest demoting the PROP to a witness-traced scenario (a `run` demonstrating the eventuality once) plus a fairness note — don't leave the user believing liveness was proven.
- **Transition properties (`over: "probes"`):** an invariant that needs the state BEFORE as well as after — "X never changes", "Y only moves one way", "DELETED is terminal" — is not expressible over a single state and must not be written off as unenforceable prose. Do **not** reach for `always(P(next(x)))`: quint compiles it to `[](x = x')`, which TLA+ rejects (it wants box-action form `[][A]_vars`) — the route that works is making the pre-state a variable, which is what the `_prev*` ghosts are. So set three fields on the invariant: `over: "probes"`, a `quint_name` to declare, and the `predicate` itself — `"groups.keys().forall(g => groups.get(g).policy == _prevGroups.get(g).policy)"`. Then regenerate (`tools/spec-probes.py <target>`), which emits the val into the probe module where those ghosts live, and `spec-record` checks it with `--init=initP --step=stepP`. The predicate lives in the area JSON, not in the module, because the module is generated and the ghosts are out of scope in the sidecar; `over: "probes"` without a `predicate` FAILs `invariant-over-probes-without-predicate` — nothing would declare the val. When a requirement's enforcing invariant seems impossible to state, check this first: the usual reason is that the property is a transition, not a state.
- **`JUSTIFIED-UNSET`** → the requirement carries a discharge (`witness.justification`, or an `enforced_by` on a requirement that is not `modality: "forbidden"`) at a `witness.status` other than `"skipped"`, and nothing consults a discharge until the status says so. The fix is the status, not a predicate — do not draft one, and do not read this as `no-predicate`. Confirm with the user that the skip is intended, set `witness.status: "skipped"`, and prefer moving prose to `modality: "forbidden"` + `witness.enforced_by`, which is checkable.
- **Invariants (INV-NNN) honesty rule:** by default `quint verify` checks invariants by **bounded** model checking. `formal_status: "verified"` therefore means *no counterexample within N steps* — NOT a proof, and the readback renders it `✓ (≤N steps)`. **N is per invariant**, recorded in `check_results.checks[].steps`: the two-pass ladder deepens what it has budget for and leaves the rest at `shallow_steps`, so two `✓` marks in the same area can rest on different depths. `check_results.max_steps` is the configured *ceiling*, not a claim that everything reached it. Read a check whose entry carries a budget note as *clean to the shallow bound, and never tried deeper* — the fix is `--budget` (or `apalache.budget_seconds`), or `--only` on the one invariant that matters, and saying so out loud beats letting a `✓ (≤3 steps)` be read as a `✓ (≤10 steps)`. When the user wants an unbounded proof for a load-bearing invariant (e.g. an auth guard), set `proof: "inductive"` on it in the area JSON; `spec-record` runs `quint verify --inductive-invariant=<quint_name>` and a pass becomes `verified-inductive` (`✓ proven`). If quint reports the invariant isn't constrained enough ("x is used before it is assigned"), that's an honest non-proof — help the user strengthen the predicate, don't fall back to bounded and call it proven. Suggest upgrading any `critical` invariant the reviewer would read as "always true" to inductive.
...
Settings:       max_steps=10 (shallow 3), timeout=300s/check, budget=900s/run
