...
- **Transition properties (`over: "probes"`):** an invariant that needs the state BEFORE as well as after — "X never changes", "Y only moves one way" — is not expressible over a single state and must not be written off as unenforceable prose. Set `over: "probes"` on it and give it a `quint_name` declared in the PROBE module, where the `_prev*` ghosts live; `spec-record` then checks it with `--init=initP --step=stepP`. When a requirement's enforcing invariant seems impossible to state, check this first: the usual reason is that the property is a transition, not a state.
